...
export default function Purchase() {
  ...
  const createRequest = useMutation({
    mutationFn: async () => {
      // Generate request ID
      const requestId = `PR${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`;
      ...
      <DialogContent>
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setNewRequest(prev => ({ ...prev, requester: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e) => setNewRequest(prev => ({ ...prev, items: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            disabled={!newRequest.requester || !newRequest.items || !newRequest.amount}
          >
            Submit
          </Button>
        </DialogFooter>
        ...
        {selectedRequest && (
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label>Status</Label>
                <Badge className="mt-1" variant={selectedRequest.status === "Approved" ? "default" : selectedRequest.status === "Rejected" ? "destructive" : "secondary"}>
              ...
              <div>
                <Label>Amount</Label>
                <p className="font-medium">Rp {selectedRequest.amount.toLocaleString()}</p>
              </div>
              <div className="col-span-2">
                <Label>Items</Label>
                <p className="font-medium whitespace-pre-wrap">{selectedRequest.items}</p>
              </div>
            </div>
            <div className="flex gap-2 mt-4">
              ...
              <Button
                variant={selectedRequest.status === "Rejected" ? "destructive" : "outline"}
                disabled={selectedRequest.status !== "Pending"}
